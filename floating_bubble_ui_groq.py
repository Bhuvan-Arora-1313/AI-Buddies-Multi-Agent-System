...
from buddy_magic_groq import run_buddy_system
...
if __name__ == "__main__":
    ...
    swe_ui = BuddyPopup(root, "👨‍💻 SWE Buddy (Groq)", swe_messages, swe_inputs, x_offset=100)
    pa_ui = BuddyPopup(root, "🤖 PA Buddy (Groq)", pa_messages, pa_inputs, x_offset=20)


    def toggle_pause():
        new_state = not is_paused()
        set_paused(new_state)
        pause_button.config(text="Resume" if new_state else "Pause")


    # Create your window

    root.title("Buddy Bubble")
    root.geometry("200x100")  # size of your floating window

    # Add label and button
    label = Label(root, text="Buddy Bubble")
    label.pack(pady=10)
